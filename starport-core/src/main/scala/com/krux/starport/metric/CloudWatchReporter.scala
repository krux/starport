...
import com.amazonaws.services.cloudwatch.model.{Dimension, MetricDatum, PutMetricDataRequest, PutMetricDataResult, StandardUnit, StatisticSet}
import com.codahale.metrics.{Clock, Counter, Counting, Gauge, Histogram, Meter, Metered, MetricFilter, MetricRegistry, ScheduledReporter, Snapshot, Timer}
import com.codahale.metrics.jvm.{BufferPoolMetricSet, ClassLoadingGaugeSet, FileDescriptorRatioGauge, GarbageCollectorMetricSet, MemoryUsageGaugeSet, ThreadStatesGaugeSet}
...
case class CloudWatchReporter(builder: Builder)
    extends ScheduledReporter(builder.metricRegistry,
  ...
  override def report(gauges: SortedMap[String, Gauge[Long]],
    ...
    try {
      ...
      val metricDataPartitions: Collection[List[MetricDatum]] = CollectionsUtils.partition(metricData, MAXIMUM_DATUMS_PER_REQUEST)
      val cloudWatchFutures: List[Future[PutMetricDataResult]] = new ArrayList[Future[PutMetricDataResult]](metricData.size)
      ...
      if (LOGGER.isDebugEnabled) {
        LOGGER.debug(s"Sent ${metricData.size} datums to CloudWatch. Namespace: ${namespace}, metric data ${metricData}.")
      }
    } catch {
  ...
  private def processCounter(metricName: String,
                             counter: Counting,
                             metricData: List[MetricDatum]): Unit = {
  ...
  /**
    ...
    */
  private def processMeter(metricName: String,
                           meter: Metered,
                           metricData: List[MetricDatum]): Unit = {
    val formattedRate: String = String.format("-rate [per-%s]", getRateUnit)
    stageMetricDatum(builder.oneMinuteMeanRate,
                     metricName,
                     convertRate(meter.getOneMinuteRate),
                     rateUnit,
                     "1-min-mean" + formattedRate,
                     metricData)
    stageMetricDatum(builder.fiveMinuteMeanRate,
                     metricName,
                     convertRate(meter.getFiveMinuteRate),
                     rateUnit,
                     "5-min-mean" + formattedRate,
                     metricData)
    stageMetricDatum(builder.fifteenMinuteMeanRate,
                     metricName,
                     convertRate(meter.getFifteenMinuteRate),
                     rateUnit,
                     "15-min-mean" + formattedRate,
                     metricData)
    stageMetricDatum(builder.meanRate,
                     metricName,
  ...
  /**
    ...
    */
  private def processTimer(metricName: String,
                           timer: Timer,
                           metricData: List[MetricDatum]): Unit = {
    val snapshot: Snapshot = timer.getSnapshot
    if (builder.zeroValuesSubmission || snapshot.size > 0) {
      for (percentile <- builder.percentiles) {
        val convertedDuration: Double = convertDuration(
          snapshot.getValue(percentile.quantile))
        stageMetricDatum(true,
                         metricName,
                         convertedDuration,
                         durationUnit,
...
// prevent empty snapshot from causing InvalidParameterValueException
    if (snapshot.size > 0) {
      ...
      stageMetricDatum(builder.arithmeticMean,
                       metricName,
                       convertDuration(snapshot.getMean),
                       durationUnit,
                       DIMENSION_SNAPSHOT_MEAN + formattedDuration,
                       metricData)
      stageMetricDatum(builder.stdDev,
                       metricName,
                       convertDuration(snapshot.getStdDev),
                       durationUnit,
                       DIMENSION_SNAPSHOT_STD_DEV + formattedDuration,
                       metricData)
      stageMetricDatumWithConvertedSnapshot(builder.statisticSet,
                                            metricName,
                                            snapshot,
                                            durationUnit,
                                            metricData)
  ...
  /**
    ...
    */
  private def processHistogram(metricName: String,
                               histogram: Histogram,
                               metricData: List[MetricDatum]): Unit = {
...
// prevent empty snapshot from causing InvalidParameterValueException
    if (snapshot.size > 0) {
      ...
      stageMetricDatum(builder.stdDev,
                       ...
                       metricData)
      stageMetricDatumWithRawSnapshot(builder.statisticSet,
                                      metricName,
                                      snapshot,
                                      StandardUnit.None,
                                      metricData)
  ...
  /**
    ...
    * If {@link Builder#withZeroValuesSubmission()} is {@code true}, then all values will be submitted
    */
  private def stageMetricDatum(metricConfigured: Boolean,
                               metricName: String,
                               metricValue: Double,
    ...
    if (metricConfigured && (builder.zeroValuesSubmission || metricValue > 0)) {
      ...
      metricData.add(
        new MetricDatum()
          ...
          .withStorageResolution(if (highResolution) HIGH_RESOLUTION
...
object CloudWatchReporter {
  ...
  /**
    ...
    */
  def forRegistry(metricRegistry: MetricRegistry,
  ...
  class Builder (
    ...
    def build(): CloudWatchReporter = {
      if (jvmMetrics) {
        metricRegistry.register("jvm.uptime", ManagementFactory.getRuntimeMXBean.getUptime.asInstanceOf[Gauge[Long]])
